...
class DeepSet(torch.nn.Module):
    ...
    def __init__(self, feats, classes, hidden=64):
        super().__init__()
        self.equiv1 = torch.nn.Parameter(torch.tensor([[1. for _ in range(hidden)] for _ in range(feats)]))
        self.equiv2 = torch.nn.Parameter(torch.tensor([[1. for _ in range(hidden)] for _ in range(feats)]))
    ...
    def forward(self, data):
        ...
        x = torch.matmul(x, self.equiv1) + torch.matmul(self.conv(x, edges), self.equiv2)
        x = F.relu(x/2)
