...
class RegressionTask:
    def __init__(
        ...
        self.mask = mask
        self.l1 = l1
        self.classes = min(labels.shape[1], labels.shape[0]) if classes is None else classes#int(labels.max().detach()) + 1 if classes is None else classes
        ...
        self.deviation = (lambda x, y: ((x-y)**2).sum()**0.5*y.shape[0]**0.5) if deviation is None else deviation
