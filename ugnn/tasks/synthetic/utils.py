...
class RandomGraphTask(ClassificationTask):
    def __init__(
        ...
        rni=0,  # 7 for 100 nodes (do not activate - implemented in architectures)
        de=True,  # distance encoding instead of rni
        ...
        **kwargs
        ...
        if distribution_name is not None:
            r = [value for value in labels[mask_mask]]
            from matplotlib import pyplot as plt
            import numpy as np
            plt.hist(r, bins=np.arange(min(r), max(r) + 2) - 0.5, edgecolor='black')
            plt.xticks(np.arange(min(r), max(r) + 1))
            plt.xlabel(distribution_name)
            plt.ylabel("Number of nodes across all graphs")
            plt.show()
...
class RandomGraphTaskRegression(RegressionTask):
    def __init__(
        self,
        nodes: int = 20,
        max_density: float = 0.1,
